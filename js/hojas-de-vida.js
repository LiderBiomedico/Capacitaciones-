...
(function () {
  ...
  const hvState = window.__HSLV_HV_STATE || (window.__HSLV_HV_STATE = {
    records: [],
    allRecords: [],          // cache completo del inventario (todas las paginas)
    filtered: [],
    loaded: false,           // true cuando el cache completo ya se cargo
    loading: false,
    currentOffset: null,
    searchQuery: '',
    searchTimeout: null,
    fServicio: '',           // filtro por Servicio
    fMarca: '',              // filtro por Marca
    fModelo: '',             // filtro por Modelo
    currentPage: 0,
    pageSize: 20,
    currentRecord: null,
  });

  // Lee el valor de un campo probando varias variantes de nombre
  function fv(f, ...keys) {
    for (const k of keys) {
      if (f && f[k] !== undefined && f[k] !== null && String(f[k]).trim() !== '') return String(f[k]).trim();
    }
    return '';
  }
  ...
  // ── LOAD ──────────────────────────────────────────────────────────────────
  // Carga TODO el inventario una sola vez (paginando) y lo guarda en cache.
  // Luego el buscador y los filtros trabajan 100% del lado del cliente.

  async function loadHojasVida(force) {
    const listEl = document.getElementById('hvListBody');
    if (!listEl) return;
    if (hvState.loading) return;

    // Si ya esta cacheado y no se forzo recarga, solo re-filtra y re-pinta.
    if (hvState.loaded && !force) { hvApplyFilters(); return; }

    hvState.loading = true;
    hvState.currentPage = 0;
    listEl.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:24px;color:#607d8b;">⏳ Cargando inventario completo...</td></tr>`;
    ...
    try {
      ...
      do {
        ...
        guard++;
        // Feedback de progreso mientras carga
        listEl.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:24px;color:#607d8b;">⏳ Cargando inventario completo... (${all.length} equipos)</td></tr>`;
      } while (offset && guard < 60);

      hvState.allRecords = all;
      hvState.records = all;
      hvState.loaded = true;
      ...
      hvState.loading = false;
    }
  }

  // ── FILTROS DESPLEGABLES ──────────────────────────────────────────────────
  // Llena los <select> de Servicio / Marca / Modelo con los valores reales
  // que existen en el inventario (ordenados, sin duplicados).

  function populateHvFilters() {
    const build = (selectId, keys, currentVal) => {
      const sel = document.getElementById(selectId);
      if (!sel) return;
      const set = new Set();
      hvState.allRecords.forEach(r => {
        const val = fv(r.fields || {}, ...keys);
        if (val) set.add(val);
      });
      const opts = Array.from(set).sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }));
      const firstLabel = sel.dataset.placeholder || sel.options[0]?.text || 'Todos';
      sel.innerHTML = `<option value="">${firstLabel}</option>` +
        opts.map(o => `<option value="${esc(o)}">${esc(o)}</option>`).join('');
      // Conservar la seleccion previa si sigue existiendo
      if (currentVal && opts.indexOf(currentVal) !== -1) sel.value = currentVal;
    };

    build('hvFilterServicio', ['Servicio', 'SERVICIO'], hvState.fServicio);
    build('hvFilterMarca',    ['Marca', 'MARCA'],       hvState.fMarca);
    build('hvFilterModelo',   ['Modelo', 'MODELO'],     hvState.fModelo);
  }

  // Se llama cuando el usuario cambia cualquiera de los <select>
  function hvFilterChange() {
    const sv = document.getElementById('hvFilterServicio');
    const mc = document.getElementById('hvFilterMarca');
    const md = document.getElementById('hvFilterModelo');
    hvState.fServicio = sv ? sv.value : '';
    hvState.fMarca    = mc ? mc.value : '';
    hvState.fModelo   = md ? md.value : '';
    hvApplyFilters();
  }

  // Limpia buscador + todos los filtros
  function hvClearFilters() {
    hvState.searchQuery = '';
    hvState.fServicio = '';
    hvState.fMarca = '';
    hvState.fModelo = '';
    const inp = document.getElementById('hvSearchInput');
    if (inp) inp.value = '';
    ['hvFilterServicio', 'hvFilterMarca', 'hvFilterModelo'].forEach(id => {
  ...
  // ── APLICAR FILTROS ───────────────────────────────────────────────────────
  // Combina el texto libre + los 3 desplegables sobre TODO el inventario.

  function hvApplyFilters() {
    const q = (hvState.searchQuery || '').toLowerCase().trim();
    const sv = (hvState.fServicio || '').toLowerCase();
    const mc = (hvState.fMarca || '').toLowerCase();
    const md = (hvState.fModelo || '').toLowerCase();

    hvState.filtered = hvState.allRecords.filter(r => {
      const f = r.fields || {};
      // Filtros exactos por desplegable
      if (sv && fv(f, 'Servicio', 'SERVICIO').toLowerCase() !== sv) return false;
      if (mc && fv(f, 'Marca', 'MARCA').toLowerCase() !== mc) return false;
      if (md && fv(f, 'Modelo', 'MODELO').toLowerCase() !== md) return false;
      // Texto libre sobre varios campos
      if (q) {
        const hay = [
          'Equipo','EQUIPO','Marca','MARCA','Modelo','MODELO','Serie','SERIE',
          'Numero de Placa','PLACA','Servicio','SERVICIO','Item','ITEM','Sede','SEDE'
  ...
  function renderHvList() {
    ...
    const total = hvState.filtered.length;

    // Contador: muestra resultados filtrados vs total del inventario
    const countEl = document.getElementById('hvCount');
    if (countEl) {
      const tieneFiltro = hvState.searchQuery || hvState.fServicio || hvState.fMarca || hvState.fModelo;
      countEl.textContent = tieneFiltro
        ? `${total} de ${hvState.allRecords.length} equipos`
        : `${total} equipos`;
    }
    ...
    // Paginacion del lado del cliente sobre el resultado filtrado
    const start = hvState.currentPage * hvState.pageSize;
    const pageRecords = hvState.filtered.slice(start, start + hvState.pageSize);

    listEl.innerHTML = pageRecords.map(record => {
  ...
  function hvSearch() {
    ...
    hvState.searchTimeout = setTimeout(() => {
      ...
      hvState.searchQuery = (el ? el.value : '').toLowerCase().trim();
      // Si todavia no se cargo el inventario completo, cargarlo primero
      if (!hvState.loaded) { loadHojasVida(true); return; }
      hvApplyFilters();
    }, 250);
  }

  // ── PAGINATION ────────────────────────────────────────────────────────────
  // Ahora la paginacion es 100% del lado del cliente sobre el set filtrado.

  function updateHvPagination() {
    const prev = document.getElementById('hvPrevBtn');
    const next = document.getElementById('hvNextBtn');
    const totalPages = Math.max(1, Math.ceil(hvState.filtered.length / hvState.pageSize));
    if (prev) prev.disabled = hvState.currentPage <= 0;
    if (next) next.disabled = hvState.currentPage >= totalPages - 1;
  }

  function hvNextPage() {
    const totalPages = Math.ceil(hvState.filtered.length / hvState.pageSize);
    if (hvState.currentPage < totalPages - 1) {
      hvState.currentPage++;
      renderHvList();
      updateHvPagination();
    }
  ...
  window.hvFilterChange = hvFilterChange;
  window.hvClearFilters = hvClearFilters;
