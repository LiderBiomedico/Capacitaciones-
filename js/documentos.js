...
(function () {
  ...
  const DOC_FIELDS = [
    ...
    { key: 'Mantenimiento Aprobado',      label: 'Reportes Mtto. Preventivo',     icon: '🛡️', color: '#1b5e20', bg: '#e8f5e9', stripApprovalPrefix: true },
  ...
  // Normaliza un nombre de servicio para comparar de forma segura:
  // quita espacios al inicio/fin, colapsa espacios internos y pasa a minusculas.
  // Evita el bug de "Sin resultados" cuando Airtable tiene espacios o mayus/minus inconsistentes.
  function normSvc(s) { return String(s == null ? '' : s).trim().replace(/\s+/g, ' ').toLowerCase(); }

  // Lee el servicio de un registro probando ambas variantes de nombre de campo.
  function recSvc(r) { const f = r && r.fields || {}; return f['Servicio'] || f['SERVICIO'] || ''; }
  ...
  function updateDocStats() {
    // Si hay servicio filtrado, las KPIs reflejan solo ese servicio
    let scope = docState.records;
    if (docState.filterServicio && docState.filterServicio !== 'TODOS') {
      const target = normSvc(docState.filterServicio);
      scope = docState.records.filter(r => normSvc(recSvc(r)) === target);
    }
  ...
  function renderDocList() {
    ...
    const svcFilterNorm = (docState.filterServicio || 'TODOS') === 'TODOS' ? null : normSvc(docState.filterServicio);
    docState.filtered = docState.records.filter(r => {
      ...
      // Filtro por servicio (dropdown) — comparacion normalizada
      if (svcFilterNorm && normSvc(recSvc(r)) !== svcFilterNorm) return false;
  ...
  function buildDocCard(att, df) {
    const rawName = att.filename || att.name || 'documento.pdf';
    const name    = df.stripApprovalPrefix ? rawName.replace(/^(?:APROBADO|RECHAZADO)_[^_]+_/, '') : rawName;
    const url   = att.url || '';
  ...
  // ── FILTRO POR SERVICIO ───────────────────────────────────────────────────
  function populateServicioFilter() {
    const sel = document.getElementById('docServicioFilter');
    if (!sel) return;
    // Agrupa por servicio normalizado: si Airtable tiene "Consulta Externa" y
    // "consulta externa " (con espacio), cuentan como UN solo servicio.
    const groups = {}; // normKey -> { label, count }
    docState.records.forEach(r => {
      const raw = String(recSvc(r) || '').trim().replace(/\s+/g, ' ');
      if (!raw) return;
      const key = raw.toLowerCase();
      if (!groups[key]) groups[key] = { label: raw, count: 0 };
      groups[key].count++;
    });
    const sorted = Object.values(groups).sort((a, b) => a.label.localeCompare(b.label, 'es'));
    const current = docState.filterServicio || 'TODOS';
    sel.innerHTML = '<option value="TODOS">📍 Todos los servicios (' + docState.records.length + ')</option>' +
      sorted.map(g => `<option value="${esc(g.label)}">${esc(g.label)} (${g.count})</option>`).join('');
    // Restaurar la seleccion previa comparando de forma normalizada
    if (current === 'TODOS') {
      sel.value = 'TODOS';
    } else {
      const match = sorted.find(g => normSvc(g.label) === normSvc(current));
      sel.value = match ? match.label : 'TODOS';
      docState.filterServicio = match ? match.label : 'TODOS';
    }
    sel.classList.toggle('has-filter', sel.value !== 'TODOS');
  }

  window.docFilterServicio = function(value) {
    docState.filterServicio = value || 'TODOS';
    const sel = document.getElementById('docServicioFilter');
    if (sel) sel.classList.toggle('has-filter', docState.filterServicio !== 'TODOS');
    const badge = document.getElementById('docPanelServicioBadge');
    if (badge) {
      if (docState.filterServicio !== 'TODOS') {
        badge.textContent = '📍 ' + docState.filterServicio;
        badge.style.display = '';
      } else {
        badge.style.display = 'none';
      }
    }
    // Si el equipo seleccionado no pertenece al servicio, limpiar selección
    if (docState.selected && docState.filterServicio !== 'TODOS') {
      if (normSvc(recSvc(docState.selected)) !== normSvc(docState.filterServicio)) {
